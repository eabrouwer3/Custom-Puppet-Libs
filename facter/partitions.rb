# Credit for these facts goes to github user @kwilczynski
...
if Facter.value(:kernel) == 'Linux'
  ...
  not_real_hard_drives = [ 'sr' ]

  hard_drives = disks.sort.uniq
  hard_drives.each do |v|
    if not_real_hard_drives.include?(v)
      hard_drives -= [v]
    end
  end

  Facter.add('hard_drives') do
    confine :kernel => :linux
    setcode { hard_drives.sort.uniq.join(',') }
  end
